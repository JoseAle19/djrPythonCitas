...
def create_appointment(request):
    ...
    appointmentDate = AppointmentSerializer(PatientAppointmentModel.objects.all(
    ).filter(dateAppointment=request.data['dateAppointment']), many=True)
...
def list_appointment(request, finish):
    appointment = AppointmentSerializer(
        PatientAppointmentModel.objects.all().filter(statustAppointment=True), many=True)
    if finish == 1:
        appointment = AppointmentSerializer(
            PatientAppointmentModel.objects.all().filter(statustAppointment=False), many=True)
        return Response({
            'Status': True,
...
@api_view(['GET'])
def list_appointmentFlutter(request, finish):
    appointment = AppointmentSerializer(
        PatientAppointmentModel.objects.all().filter(statustAppointment=True), many=True)
    if finish == 1:
